...
func (s *PeriodService) OpenPeriod(ctx context.Context, req OpenPeriodRequest) (*domain.Period, int, error) {
	...
	`, period.PeriodStart, period.PeriodEnd).Scan(&period.ID, &period.CreatedAt); err != nil {
		return nil, 0, fmt.Errorf("create period: %w", err)
	}
	...
	`, req.PeriodStart)
	if err != nil {
		return nil, 0, fmt.Errorf("fetch subscriptions: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&sr.subscriptionID, &sr.serviceID, &sr.hasMeter, &connectedAt, &disconnectedAt); err != nil {
			return nil, 0, fmt.Errorf("scan subscriptions: %w", err)
		}
		subs = append(subs, sr)
	}
	rows.Close()
	...
	for _, sub := range subs {
		...
		if err != nil {
			// No active tariff — skip this subscription
			continue
		}
		...
		if sub.hasMeter {
			...
			// Flat-rate: quantity = 1, amount = price
			quantity = 1
		...
		if err != nil {
			return nil, 0, fmt.Errorf("insert calculation for sub %d: %w", sub.subscriptionID, err)
		}
		generated++
	}
